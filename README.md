...
-> JAX parameter calibration / FerroX phase-field simulation
...
JAX/FerroX 仿真环境为可选计算依赖，不强制普通网页环境安装：
...
本地 FerroX 使用 `NOACC + MPI OFF` 的 CPU 微型自检版；大规模 MPI/GPU 算例只在人工确认参数、费用和云端队列后运行。JAX 默认用于 Landau 自由能自动微分、敏感性与参数校验；FerroX 用于 TDGL/Poisson 域结构仿真。冒烟测试不是 HZO 物理结论。
