...
from backend.services.simulation_runtime import prepare_ferrox_job_package
...
def _write_job_package(row: pd.Series, jobs_dir: Path, execution_mode: str) -> tuple[str, Path, dict[str, Any]]:
    ...
    task_family = str(row.get("task_family", ""))
    if task_family == "phase_stability":
        _write_phase_stability_package(work_dir, manifest)
    elif task_family == "phase_field_switching":
        prepare_ferrox_job_package(work_dir, manifest)
    else:
...
def import_computation_results(
    ...
    with connect(db_path) as conn:
        ...
        for index, row in normalized.iterrows():
            ...
            inserted += 1
        status = "results_imported" if quality_counts.get("needs_review", 0) == 0 else "needs_review"
        conn.execute(
            """
