...
def render_html(data: dict[str, object], refresh: int) -> bytes:
    status = "已完成" if data.get("completed") else "运行中"
    if data.get("extraction_done") and not data.get("completed"):
        status = "抽取完成，正在后处理"
    if data.get("missing"):
        status = "等待日志"
...
<head>
  ...
  <style>
    ...
    .fill {{ width: {percent}%; height: 100%; background: linear-gradient(90deg, #5bd489, #69a7ff); }}
...
<main>
  ...
  <div class="muted">状态：{html.escape(status)} · 每 {refresh} 秒自动刷新 · 日志：{log_path}</div>
  <div class="bar"><div class="fill"></div></div>
  <div class="grid">
    <div class="card"><div class="label">进度</div><div class="value">{percent}%</div></div>
    <div class="card"><div class="label">已处理 chunk</div><div class="value">{data.get("processed", 0)} / {data.get("total", 0)}</div></div>
    <div class="card"><div class="label">新增候选</div><div class="value">{data.get("candidates", 0)}</div></div>
    <div class="card"><div class="label">空结果</div><div class="value">{data.get("empty", 0)}</div></div>
    <div class="card"><div class="label">模型失败</div><div class="value">{data.get("llm_failed", 0)}</div></div>
    <div class="card"><div class="label">程序错误</div><div class="value">{data.get("errors", 0)}</div></div>
    <div class="card"><div class="label">已用时间</div><div class="value">{data.get("elapsed", "0s")}</div></div>
    <div class="card"><div class="label">预计剩余</div><div class="value">{data.get("eta", "估算中")}</div></div>
  </div>
  <h2>实时日志</h2>
  <pre>{tail}</pre>
</main>
...
def make_handler(log_path: Path | None, log_dir: Path, total: int, refresh: int):
    class ProgressHandler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:
            ...
            if parsed.path not in {"/", "/progress"}:
                self.send_error(404)
                return
