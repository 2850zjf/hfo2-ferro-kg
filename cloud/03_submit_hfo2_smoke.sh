...
if [[ $# -gt 0 ]]; then
  cat >&2 <<'EOF'
This legacy multi-phase route is dry-run only and refuses paid submission.
Use computations/phase_strain_pilot/tefs_single_job_pilot_v0_1 instead.
EOF
  exit 64
fi
...
echo "Submission mode: legacy dry-run only (paid submission disabled)"
...
cmd = (
    ...
    "exec timeout --signal=TERM --kill-after=60s 30m bash run_vasp_two_step.sh"
)
payload = {
    ...
    # Job 151024 proved False is POSTPAID_BY_HOUR in the active TEFS service,
    # despite the opposite mapping in the CLI help. This route remains dry-run.
    "spot_paid": False,
    "public_ip": False,
...
PY
  ...
  echo "== $phase =="
  cat "$json_path"
  echo "Dry-run descriptor only. Do not submit this legacy multi-phase route."
done

echo
echo "Use the reviewed single-job pilot for any separately approved submission."
